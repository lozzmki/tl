local _tl_compat; if (tonumber((_VERSION or ''):match('[%d.]*$')) or 0) < 5.3 then local p, m = pcall(require, 'compat53.module'); if p then _tl_compat = m end end; local assert = _tl_compat and _tl_compat.assert or assert; local debug = _tl_compat and _tl_compat.debug or debug; local io = _tl_compat and _tl_compat.io or io; local ipairs = _tl_compat and _tl_compat.ipairs or ipairs; local math = _tl_compat and _tl_compat.math or math; local _tl_math_maxinteger = math.maxinteger or math.pow(2, 53); local os = _tl_compat and _tl_compat.os or os; local package = _tl_compat and _tl_compat.package or package; local pairs = _tl_compat and _tl_compat.pairs or pairs; local string = _tl_compat and _tl_compat.string or string; local table = _tl_compat and _tl_compat.table or table; local _tl_table_unpack = unpack or table.unpack
...
parse_record_body = function(ps, i, def, node)
   ...
   while not (ps.tokens[i].kind == "$EOF$" or ps.tokens[i].tk == "end") do
      ...
      elseif ps.tokens[i].tk == "implements" and ps.tokens[i + 1].tk ~= ":" then
         ...
         i, t = parse_type(ps, i)
         if not t then
            fail(ps, i, "expect a record type")
         end
...
local function init_globals(lax)
   ...
   a_type({ typename = "function", args = TUPLE({}), rets = TUPLE({ INTEGER, a }) }),
      ...
      ["loadstring"] = a_type({ typename = "function", args = TUPLE({ UNION({ STRING, LOAD_FUNCTION }), OPT(STRING), OPT(STRING), OPT(TABLE) }), rets = TUPLE({ FUNCTION, STRING }) }),
...
local function tl_package_loader(module_name)
   ...
   if found_filename then
      ...
      local chunk, err = loadstring(code, "@" .. found_filename, "t")
      if chunk then
         return function()
            local ret = chunk()
...
tl.load = function(input, chunkname, mode, ...)
   ...
   return loadstring(code, chunkname, mode, ...)
end
